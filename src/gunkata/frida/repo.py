...
class ServerRepo:
    ...
    Args:
        ...
        settings: Whether a missing archive may be fetched automatically,
    ...
    Design:
        ...
        a version is supplied. A missing archive is refused by default; setting
        ``settings.autodownload_server_binary`` fetches it through downloader
        instead, the one place this otherwise network-free class touches the
        network.
    """

    _ABI_PROP = "ro.product.cpu.abi"
    _URL_TEMPLATE = "https://github.com/frida/frida/releases/download/{version}/{name}"
    ...
    def extracted(
        ...
        Raises:
            ...
            ServerAssetError: The matching archive is missing from the repo and
                settings.autodownload_server_binary is not set.
    ...
    def _archive(self, version: str, arch: Arch) -> Path:
        ...
        if archive.is_file():
            return archive
        if not self._settings.autodownload_server_binary:
            raise ServerAssetError(
                f"no {name} in frida repo {self._repo}; place it there yourself, "
                "or set GUNKATA_FRIDA_AUTODOWNLOAD_SERVER_BINARY=1 to fetch it "
                "from frida's GitHub releases automatically"
            )
        self._repo.mkdir(parents=True, exist_ok=True)
        url = self._URL_TEMPLATE.format(version=version, name=name)
